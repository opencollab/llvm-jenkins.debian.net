...
DISTRO="stretch buster bullseye unstable bionic focal groovy hirsute impish"
...
for d in $DISTRO; do
    ...
    if test "$d" == "bionic" -o "$d" == "focal" -o "$d" == "groovy" -o "$d" == "hirsute" -o "$d" == "impish" -o "$d" == "jammy"; then
...
for d in $DISTRO; do
    ...
    for v in $VERSION; do
        ...
        if test -z "$USE_SCRIPT"; then
            ...
            if test "$d" != "unstable" -a "$d" != "jammy"; then
                PKG="$PKG python"
            fi
